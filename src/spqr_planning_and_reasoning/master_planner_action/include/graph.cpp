...
void Graph::exportGraph(){
	FILE *fd;
	//write mode file
	fd=fopen("/home/luca/catkin_ws/src/Robocup@Work/spqr_topological_navigation/graphs/graph.txt", "w");
	if( fd==NULL ) {
		perror("Error I/O");
		exit(1);
	}
...
pGraph Graph::importGraph(){
	...
	//open file graph
	std::ifstream Graph_file("/home/luca/catkin_ws/src/Robocup@Work/spqr_planning_and_reasoning/master_planner_action/config/graph/graph.txt");
	if (Graph_file){
		std::string line;
