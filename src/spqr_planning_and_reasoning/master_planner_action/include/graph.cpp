...
void Graph::exportGraph(){
	FILE *fd;
	//write mode file
	fd=fopen("/home/francesco/planning_ws/src/spqr_planning_and_reasoning/master_planner_action/config/graph/graph.txt", "w");
	if( fd==NULL ) {
		perror("Error I/O");
		exit(1);
	}
...
pGraph Graph::importGraph(){
	...
	//open file graph
	std::ifstream Graph_file("/home/francesco/planning_ws/src/spqr_planning_and_reasoning/master_planner_action/config/graph/graph.txt");
	if (Graph_file){
		std::string line;
...
/*
std::list<int> Graph::findPathAstar(int id_start, int id_goal)
{
	Astar* astar = new Astar(id_start, id_goal, this);
	return astar->findPathAstar();
}*/
