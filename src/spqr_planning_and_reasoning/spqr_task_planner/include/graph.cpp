...
void Graph::exportGraph(){
	FILE *fd;
	//write mode file
	fd=fopen("/home/francesco/pinguin_ws/src/spqr_topological_navigation/graphs/graph.txt", "w");
	if( fd==NULL ) {
		perror("Error I/O");
		exit(1);
	}
...
pGraph Graph::importGraph(){
	...
	//open file graph
	std::ifstream Graph_file("/home/francesco/pinguin_ws/src/spqr_topological_navigation/graphs/graph.txt");
	if (Graph_file){
		std::string line;
