...
void drawNode(callBack_userdata*_userdata){
  ...
  int slots_pose_y = 400;
  int legend_pose_y = -70;

  cv::putText(inputMap, "slot 1", cv::Point(startX+20, startY-10+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  cv::putText(inputMap, "slot 2", cv::Point(startX+23+width+5, startY-10+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  cv::putText(inputMap, "slot 3", cv::Point(startX+25+2*width+10, startY-10+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  cv::rectangle(inputMap, cv::Point(startX, startY+slots_pose_y), cv::Point(startX+width, startY+height+slots_pose_y), slots_info.at(0).first, 3);//slot1
  cv::rectangle(inputMap, cv::Point(startX+width+dist, startY+slots_pose_y), cv::Point(startX+2*width+dist, startY+height+slots_pose_y), slots_info.at(1).first, 3);//slot2
  cv::rectangle(inputMap, cv::Point(startX+2*width+2*dist, startY+slots_pose_y), cv::Point(startX+3*width+2*dist, startY+height+slots_pose_y), slots_info.at(2).first, 3);//slot3

  cv::putText(inputMap, slots_info.at(0).second, cv::Point(startX+20, startY+35+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(100, 100, 100), 1, CV_AA);
  cv::putText(inputMap, slots_info.at(1).second, cv::Point(startX+23+width+5, startY+35+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(100, 100, 100), 1, CV_AA);
  cv::putText(inputMap, slots_info.at(2).second, cv::Point(startX+25+2*width+10, startY+35+slots_pose_y),
              CV_FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(100, 100, 100), 1, CV_AA);


  cv::putText(inputMap, "Legend:", cv::Point(startX, startY+100+legend_pose_y), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 2, CV_AA);

  int step = 60;
  cv::putText(inputMap, "Current robot pose:", cv::Point(startX, startY+150+legend_pose_y), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  circle(inputMap, cv::Point(startX+350, startY+142+legend_pose_y), dim_circle*scale, cv::Scalar(255, 0, 0), -1);

  cv::putText(inputMap, "Robot poses:", cv::Point(startX, startY+150+step+legend_pose_y), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  circle(inputMap, cv::Point(startX+240, startY+142+step+legend_pose_y), dim_circle*scale, cv::Scalar(205, 205, 205), -1);

  cv::putText(inputMap, "Take:", cv::Point(startX, startY+150+2*step+legend_pose_y), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  circle(inputMap, cv::Point(startX+120, startY+142+2*step+legend_pose_y), dim_circle*scale, cv::Scalar(32, 128, 0), -1);

  cv::putText(inputMap, "Drop:", cv::Point(startX, startY+150+3*step+legend_pose_y), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(100, 100, 100), 1, CV_AA);
  circle(inputMap, cv::Point(startX+120, startY+142+3*step+legend_pose_y), dim_circle*scale, cv::Scalar(0, 0, 255), -1);

  cv::line(inputMap, cv::Point(startX-50, 0), cv::Point(startX-50, scale*inputMap.rows), cv::Scalar(0, 0, 0), 2);

  _userdata->logo.copyTo(inputMap(cv::Rect(startX+width/1.5,startY+150+3*step+legend_pose_y+200,_userdata->logo.cols, _userdata->logo.rows)));

  cv::imshow("Map", inputMap);
}
void draw(int event, callBack_userdata*_userdata , int action_showed, int gap, int event_bk){
...
//      --action_showed;
      ...
      draw(event, _userdata , action_showed-1, gap, event_bk);
    ...
    if (event == cv::EVENT_LBUTTONDOWN ) {
      ...
      if (action_showed < actions.size()) {
        draw(event, _userdata , action_showed, gap, event_bk);
        ++action_showed;
      }
  }
...
//  cout<<"map "<<map_img.channels()<<" logo "<<userdata.logo.channels()<<endl;
